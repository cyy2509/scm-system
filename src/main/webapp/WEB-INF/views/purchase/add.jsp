...
<jsp:include page="../common/header.jsp">
    <jsp:param name="title" value="创建采购订单"/>
</jsp:include>
<jsp:include page="../common/menu.jsp"/>

<el-card class="form-card" shadow="never" style="max-width:900px">
    <div slot="header"><h3>创建采购订单</h3></div>

    <form id="orderForm">
        <el-form label-position="top" size="small">
            <el-form-item label="选择供应商" required>
                <select id="supplierId" style="width:100%;height:32px;border:1px solid #DCDFE6;border-radius:4px;padding:0 10px;color:#606266;font-size:13px;" required>
            ...
            <el-form-item>
                ...
                <el-button icon="el-icon-back" @click="location.href='${ctx}/purchase/list'">返回</el-button>
            </el-form-item>
        </el-form>
    </form>
</el-card>
...
(function() {
    ...
    if (app) {
        ...
        app.$set(app, 'onProductChange', function(row) {
            ...
            if (p) { row.price = parseFloat(p.price).toFixed(2); }
            app.calcAmount(row);
        });
        ...
        app.$set(app, 'submitOrder', function() {
            ...
            if (!supplierId) { app.$message.warning('请选择供应商'); return; }
            ...
            $.ajax({
                url: '${ctx}/purchase/add', type: 'post', contentType: 'application/json', data: JSON.stringify(order),
                success: function(result) {
                    if (result === 'success') { app.$message.success('采购订单创建成功！'); setTimeout(function() { location.href = '${ctx}/purchase/list'; }, 800); }
