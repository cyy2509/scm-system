...
<jsp:include page="../common/header.jsp">
    <jsp:param name="title" value="采购订单详情"/>
</jsp:include>
<jsp:include page="../common/menu.jsp"/>

<el-card class="detail-card" shadow="never">
    <div slot="header"><h3>采购订单详情</h3></div>
    ...
    <el-descriptions :column="2" border size="medium" style="margin-bottom:24px">
        ...
        <el-descriptions-item label="状态">
            <el-tag size="small" type="<c:choose><c:when test='${order.orderStatus == 0}'>warning</c:when><c:when test='${order.orderStatus == 1}'>success</c:when><c:when test='${order.orderStatus == 2}'>danger</c:when></c:choose>">
                <c:choose><c:when test="${order.orderStatus == 0}">待审核</c:when><c:when test="${order.orderStatus == 1}">已通过</c:when><c:when test="${order.orderStatus == 2}">已驳回</c:when></c:choose>
            </el-tag>
    ...
    <div class="action-buttons">
        <el-button icon="el-icon-back" @click="location.href='${ctx}/purchase/list'">返回列表</el-button>
    </div>
</el-card>
